// **********************************************************
// Assignment2:
// Student1: Hao Liang
// UTORID user_name: liangh21
// UT Student #: 1002100102
// Author: Hao Liang
//
// Student2:Shinan Liu
// UTORID user_name:liushina
// UT Student #:1002916936
// Author:Shinan Liu
//
// Student3: Jiasong Liang
// UTORID user_name: liang101
// UT Student #: 1004203337
// Author: Jiasong Liang
//
// Student4:Zhiming Chen
// UTORID user_name:chenz145
// UT Student #:1002917016
// Author:Zhiming Chen
//
//
// Honor Code: I pledge that this program represents my own
// program code and that I have coded on my own. I received
// help from no one in designing and debugging my program.
// I have also read the plagiarism section in the course info
// sheet of CSC B07 and understand the consequences.
// *********************************************************
package test;

import SystemAndOperation.*;
import static org.junit.Assert.assertEquals;
import java.io.*;
import org.junit.*;

public class NumberTest {

    private OperateSystem os;

    private final InputStream systemIn = System.in;
    private final PrintStream systemOut = System.out;

    private ByteArrayInputStream testIn;
    private ByteArrayOutputStream testOut;

    @Before
    public void setUpOutput() {
        os = OperateSystem.createNewOperateSystem();
        testOut = new ByteArrayOutputStream();
        System.setOut(new PrintStream(testOut));
    }

    private String getOutput() {
        return testOut.toString();
    }

    @After
    public void restoreSystemInputOutput() {
        System.setIn(systemIn);
        System.setOut(systemOut);
    }

    @Test
    public void testWithValidNumber() {
        String order1 = "mkdir a1";
        String order2 = "mkdir a2";
        String order3 = "cd a1";
        String order4 = "!2";
        String order5 = "ls";
        String exp = "a2\r\n";
        os.setInput(order1, os);
        os.setInput(order2, os);
        os.setInput(order3, os);
        os.setInput(order4, os);
        os.setInput(order5, os);
        assertEquals(exp, getOutput());
    }

    @Test
    public void testWithNumberOutOfRange() {
        String order1 = "!100";
        String exp = "Number of range, please try again\r\n";
        os.setInput(order1, os);
        assertEquals(exp, getOutput());
    }

    @Test
    public void testWithNotNumber() {
        String order1 = "!QWRAAsdfafd";
        String exp = "Not a integer, please try again\r\n";
        os.setInput(order1, os);
        assertEquals(exp, getOutput());
    }

    @Test
    public void testWithMultiStatement() {
        String order1 = "!10 123 1231";
        String exp = "Not a integer, please try again\r\n";
        os.setInput(order1, os);
        assertEquals(exp, getOutput());
    }

}
